...
namespace GeDiM
{
    ...
    const Output::ExitCodes Intersector2D1D::ComputeIntersection()
    {
        ...
        double complanarTest = planeNormal.dot(lineOrigin);

        if (abs(parallelTest) < toleranceParallelism)
        {
            if (abs(complanarTest - planeTranslation) < toleranceIntersection)
            {
        ...
        else
        {
            ...
            intersectionParametricCoordinate = (planeTranslation - complanarTest) / parallelTest;
